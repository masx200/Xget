...
describe("Platform Configuration", () => {
  ...
  describe("Unified Transform Function", () => {
    ...
    it("should transform GitHub Gist paths correctly", () => {
      ...
      expect(transformPath("/gist/username/gist-id.git", "gist")).toBe(
        "/username/gist-id.git",
      );
    });
    ...
    it("should transform GitLab paths correctly", () => {
      expect(
        transformPath(
          "/gl/gitlab-org/gitlab/-/archive/master/gitlab-master.zip",
          "gl",
        ),
      ).toBe(
    ...
    it("should transform SourceForge paths correctly", () => {
      expect(
        transformPath(
          "/sf/projects/sevenzip/files/7-Zip/23.01/7z2301-x64.exe/download",
          "sf",
        ),
    ...
    it("should transform Gitea paths correctly", () => {
      expect(transformPath("/gitea/gitea/gitea/archive/master.zip", "gitea"))
        .toBe(
    ...
    it("should transform Codeberg paths correctly", () => {
      expect(
        transformPath(
          "/codeberg/forgejo/forgejo/archive/forgejo.zip",
          "codeberg",
        ),
      ).toBe(
    ...
    it("should transform Hugging Face paths correctly", () => {
      expect(
        transformPath(
          "/hf/microsoft/DialoGPT-medium/resolve/main/config.json",
          "hf",
        ),
      ).toBe(
      ...
      expect(transformPath("/hf/datasets/squad/resolve/main/train.json", "hf"))
        .toBe(
    ...
    it("should transform Civitai paths correctly", () => {
      ...
      expect(transformPath("/civitai/api/v1/model-versions/1318", "civitai"))
        .toBe(
          "/api/v1/model-versions/1318",
        );

      expect(transformPath("/civitai/api/download/models/1105", "civitai"))
        .toBe(
    ...
    it("should transform PyPI paths correctly", () => {
      expect(
        transformPath(
          "/pypi/packages/source/r/requests/requests-2.31.0.tar.gz",
          "pypi",
        ),
      ).toBe(
      ...
      expect(transformPath("/pypi/simple/requests/", "pypi")).toBe(
        "/simple/requests/",
      );
    });
    ...
    it("should transform PyPI files paths correctly", () => {
      expect(
        transformPath(
          "/pypi/files/packages/source/r/requests/requests-2.31.0.tar.gz",
          "pypi-files",
        ),
    ...
    it("should transform conda default channel paths correctly", () => {
      expect(
        transformPath("/conda/pkgs/main/linux-64/numpy-1.24.3.conda", "conda"),
      ).toBe(
    ...
    it("should transform conda community channel paths correctly", () => {
      expect(
        transformPath(
          "/conda/community/conda-forge/linux-64/repodata.json",
          "conda-community",
        ),
    ...
    it("should transform container registry paths correctly", () => {
      expect(
        transformPath(
          "/cr/ghcr/v2/nginxinc/nginx-unprivileged/manifests/latest",
          "cr-ghcr",
        ),
      ).toBe("/v2/nginxinc/nginx-unprivileged/manifests/latest");

      expect(
        transformPath("/cr/gcr/v2/distroless/base/manifests/latest", "cr-gcr"),
      ).toBe(
  ...
  describe("Jenkins Plugin Support", () => {
    ...
    it("should transform Jenkins paths correctly", () => {
      ...
      expect(transformPath("/jenkins/update-center.actual.json", "jenkins"))
        .toBe(
      ...
      expect(
        transformPath(
          "/jenkins/download/plugins/maven-plugin/3.27/maven-plugin.hpi",
          "jenkins",
        ),
      ...
      expect(
        transformPath("/jenkins/experimental/update-center.json", "jenkins"),
      ).toBe(
        "/experimental/update-center.json",
      );

      // Current paths - should preserve current paths
      expect(transformPath("/jenkins/current/update-center.json", "jenkins"))
        .toBe(
          "/current/update-center.json",
        );

      // Other paths - should be prefixed with current
      expect(transformPath("/jenkins/test-path", "jenkins")).toBe(
        "/current/test-path",
      );
    });
    ...
    it("should transform AI inference provider paths correctly", () => {
      const testCases = [
        ...
        },
        {
          provider: "ip-gemini",
          inputPath:
            "/ip/gemini/v1beta/models/gemini-2.5-flash:generateContent",
