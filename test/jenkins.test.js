...
describe("Jenkins Plugin Support", () => {
  describe("Update Center Transformations", () => {
    ...
    it("should redirect update-center.actual.json to current", () => {
      const result = transformPath(
        "/jenkins/update-center.actual.json",
        "jenkins",
      );
    ...
    it("should preserve current paths as-is", () => {
      const result = transformPath(
        "/jenkins/current/update-center.json",
        "jenkins",
      );
      expect(result).toBe("/current/update-center.json");
    });
  ...
  describe("Plugin Download Transformations", () => {
    it("should handle Maven plugin download", () => {
      ...
      expect(result).toBe(
        "/download/plugins/maven-plugin/3.27/maven-plugin.hpi",
      );
    });
  ...
  describe("Special Path Handling", () => {
    ...
    it("should handle paths with query parameters", () => {
      const result = transformPath(
        "/jenkins/update-center.json?version=2.401",
        "jenkins",
      );
  ...
  describe("Real-world Jenkins URLs", () => {
    const testCases = [
      ...
      },
      {
        ...
        input:
          "/jenkins/download/plugins/docker-workflow/563.vd5d2e5c4007f/docker-workflow.hpi",
        expected:
          "/download/plugins/docker-workflow/563.vd5d2e5c4007f/docker-workflow.hpi",
      },
      {
  ...
  describe("Version Compatibility", () => {
    it("should handle various plugin version formats", () => {
      ...
      versionFormats.forEach((version) => {
        const path =
          `/jenkins/download/plugins/test-plugin/${version}/test-plugin.hpi`;
        const result = transformPath(path, "jenkins");
        expect(result).toBe(
          `/download/plugins/test-plugin/${version}/test-plugin.hpi`,
        );
      });
    });
    ...
    it("should handle plugin names with special characters", () => {
      ...
      pluginNames.forEach((pluginName) => {
        const path =
          `/jenkins/download/plugins/${pluginName}/1.0.0/${pluginName}.hpi`;
        const result = transformPath(path, "jenkins");
        expect(result).toBe(
          `/download/plugins/${pluginName}/1.0.0/${pluginName}.hpi`,
        );
      });
    });
  });
